...
func (r *memoryTripRepository) seedTripsForOrg(orgID string) []domain.Trip {
	return []domain.Trip{
		{
			ID:             "TR-990",
...
func (r *memoryTripRepository) GetAll(ctx context.Context, orgID string) ([]domain.Trip, error) {
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	default:
		...
		var orgTrips []domain.Trip
		for _, t := range r.trips {
			if t.OrganizationID == orgID {
				orgTrips = append(orgTrips, t)
			}
		}

		if len(orgTrips) == 0 {
			seeded := r.seedTripsForOrg(orgID)
			r.trips = append(r.trips, seeded...)
			orgTrips = seeded
		}
...
func (r *memoryTripRepository) GetByID(ctx context.Context, orgID string, id string) (domain.Trip, error) {
	select {
	case <-ctx.Done():
...
func (r *memoryTripRepository) Update(ctx context.Context, orgID string, id string, trip domain.Trip) (domain.Trip, error) {
	select {
	case <-ctx.Done():
		return domain.Trip{}, ctx.Err()
	default:
